from utils import YelpChi_Dataloader,Amazon_Dataloader
from utils.utils import prepare_folder
from utils.evaluator import Evaluator
from logger import Logger
from cogdl.data import Graph
from cogdl.datasets import NodeDataset
from models import GCN, MLP, GAT, Grand, Graphsage, sgc, SAGE, GIN, DGI, SIGN, SAGN,prepare_feats,prepare_labels
...
def main():
    ...
    if args.model == 'sagn':
        x = prepare_feats(data, 3, norm="sym")
        y__emb, labels_with_pseudos, train_nid_with_pseudos = prepare_labels(data, 100, 3, 0.2)
        data.x = x
        data.y_emb = y__emb
        para_dict = sagn_parameters
        model_para = sagn_parameters.copy()
        model_para.pop('lr')
        model_para.pop('l2')
        model = SAGN(in_feats=data.x[0].size(-1), out_feats=nlabels, **model_para).to(device)
    ...
    for run in range(args.runs):
        ...
        torch.save(best_out, result_dir + '/{}_run{}.pt'.format(args.model, run))
